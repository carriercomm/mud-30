...
def stub_map(apps, schema_editor):
    ...
    entrance_room.save()
    study = Room(description='You are in the study. Ancient books surround you, but there\'s nothing else. The foyer is south of you')
    study.save()
    ...
    RoomActionResult(action_id=foyer_to_study.id, room_id=study.id).save()

    # create the action for going from the study to the foyer
    study_to_foyer = Action(message='You left the study and are now in the foyer again', room_id=study.id, matcher='(exit|leave study|go south)')
    study_to_foyer.save()
